...
@Composable
fun rememberMultiSelectDatePickerController(): PersianMultiSelectDatePickerController {
    return remember { PersianMultiSelectDatePickerController() }
}


class PersianMultiSelectDatePickerController {
    ...
    private var _selectedDates: MutableState<List<PDate>?> = mutableStateOf(null)
    internal val selectedDates get() = _selectedDates.value
    ...
    internal fun addOrRemoveSelectedDate(date: PDate) {
        val list = selectedDates?.toMutableList() ?: mutableListOf()
        val item = list.find { it.persianDate.startOfDay() == date.persianDate.startOfDay() }
        if (item == null) {
            list.add(date)
        } else {
            list.remove(item)
        }
        _selectedDates.value = list
    }
